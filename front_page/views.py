...
def ml_index(request):
    ...
    print(df.head()) 
    X_test = df
    loaded_model = pickle.load(open('/home/pratik/Desktop/Technical/finalized_model_7class.sav', 'rb'))
...
def excel_index(request):
    if "GET" == request.method:
        ...
        for i in range(0, len(Total_Liabilities)):
            ...
            x_quick[i] = float(cash[i].value) + float(investments[i].value) + float(current_assets[i].value) + float(receivables[i].value)
        ...
        #df.loc[len(df)] = current_ratio
        #df.loc[len(df) + 1] = quick_ratio
        #df.loc[len(df) + 2] = debt_equity
        excel_data = list()
        #excel_data1 = list()
        ...
        for i in range(1, df2.max_column - 1):
            l = []
            l1 = []
            #l = [df2.cell(row=k, column=i) for k in range(0, 3)]
            [ l.append(excel_data[p][i]) for p in range(0,3) ]
            [ l1.append(excel_data[k][i+1]) for k in range(0, 3)]
            #l1 = [df2.cell(row=k, column=i + 1) for k in range(0, 3)]
            for j in range(0, len(l)):
                if (l1[j] == 0 or l[j] == 0):
                    l[j] = 0
                else:
